...
def parse_function(filename, text, labels):
...
def augmentation_fn(image, text, labels):
...
def make_grayscale_fn(image, text, labels):
...
def get_mimic_dataset(csv_root,
    ...
    dataset = tf.data.Dataset.from_tensor_slices((image_paths, texts_tokenized, labels))
    if mode == 'train':
